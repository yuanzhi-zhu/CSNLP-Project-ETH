...
def format_CANARD_data(data, include_story):
    ...
    # get the story from QuAC dataset
    if include_story:
        context_dic = {
...
def get_coqar_train_and_dev_sets(include_story):
    ...
    # length of dev data in canard
    i = len(get_canard_dev_set(include_story)['input'])
    # split the train data, cut the same amount as dev data in canard as dev data for coqar
...
def get_canard_train_set(include_story):
    # return dictionary: formatted data: {'input' : input, 'references' : references, 'context' : context, 'answer_spans' : answers}
...
def get_mixed_train_and_dev_sets(include_story):
    ...
    # make sure the dev set of coqar and canard are the same size
...
def print_dataset_into_file(dataset, output_tokenizer, path, n = 20, predictions = None):
    # only save the first n data samples
    decoded_data = []
    for i in range(n):
        row = dataset[i]
...
class QRDataset(Dataset):
    def __init__(self,
        ...
        for c, seq, refs in zip(data['context'], data['input'], data['references']):
            # data["input"][i] is list of dialogue turns [question1, answer1, question2, answer2, ..., questionN]
            # seq is the historical questions and answers, and current question
            for y in refs:
                # start position
                start = max(-len(seq), -history_size*2-1) # history_size = 3 means 2 historical qa and current q
                assert start % 2 == 1 #as there are historical question answer paring and current question
