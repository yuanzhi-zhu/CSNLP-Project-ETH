...
def Extract_Feature(example, tokenizer, max_seq_length, doc_stride, max_query_length):
    """Loads a data file into a list of `InputBatch`s."""
    #   adaptation of hugging face's extract features to include yes, no and unknown logits
    #   https://github.com/huggingface/transformers/blob/master/src/transformers/data/processors/squad.py
    # function: squad_convert_example_to_features
    features = []
    query_tokens = []
    ...
    # sub_token id
    orig_to_tok_index = []
    # all tokens (inc. sub_tokens)
    all_doc_tokens = []
    for (i, token) in enumerate(example.doc_tokens):
        orig_to_tok_index.append(len(all_doc_tokens))
        sub_tokens = tokenizer.tokenize(token)
        # sometimes tokenizer.tokenize('Vat') --> ['va', '##t']
        for sub_token in sub_tokens:
            tok_to_orig_index.append(i)
            all_doc_tokens.append(sub_token)


    # get tok_start_position and tok_end_position for the tokenized document
    ...
    # doc_stride: The stride used when the context is 
    # too large and is split across several features.
    while start_offset < len(all_doc_tokens):
        length = len(all_doc_tokens) - start_offset
        # Without this if condition, we have always the condition for next if satisfied
        # That is: when len(all_doc_tokens) > max_tokens_for_doc we have multiple spans
        # to cover the whole document
        # max_tokens_for_doc will decrease with the increase of query_tokens
	...
	# tokens are all tokens from both queries and document
	# segment_ids for query 0 for document 1
        ...
        # rational_part
        # rational position is according to all the tokens
